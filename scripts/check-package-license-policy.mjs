...
// choosing its license rather than silently inheriting an assumed default.
...
export function checkPackageLicensePolicy(root = repoRoot) {
  ...
  for (const packagePath of walkPackageJsonFiles(root)) {
    ...
    const expected = EXPECTED_LICENSES.get(packageRelativePath);

    if (expected === undefined) {
      failures.push(
        `${packageRelativePath} is not covered by scripts/check-package-license-policy.mjs; add an explicit expected license`,
      );
    } else if (packageJson.license !== expected) {
      failures.push(
        `${packageRelativePath} declares license ${JSON.stringify(packageJson.license)}; expected ${JSON.stringify(expected)}`,
      );
    }
  }
