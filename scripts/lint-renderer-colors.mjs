 ...
 *   - `pnpm lint` runs it after the forge-kind check — forge kinds, colors,
 *     package reservation, dependency maturity, licenses, lint:boundaries,
 *     and typecheck, ordered cheapest-first so a fast failure doesn't wait on
 *     the slow ones. CI's Typecheck job invokes `pnpm lint`.
 */
