...
29 across 19 files when this was written. Reducing that is tracked separately;
...
- **`Record<ForgeKind, …>` fails loudly.** A new kind is a missing-property type
  error, so `tsc` hands you the list of tables to fill in. Six exist —
  `FORGE_CLI` (shared), `FORGE_CAPABILITIES`, `PROVIDERS`, the commit-author
  transports, the E2E fixture.
