...
function ownershipMatches(directory, callId, holderPid) {
  ...
  const baselineKey = baseline?.identityMode === "kernel-object-id" ? "objectId" : "pipeId";
  ...
  if (baseline?.status !== "sampled" || baseline.limited || baseline.accessFailures || baseline.nameFailures ||
    !Array.isArray(baseline.handles) || baseline.handles.some(row => !row[baselineKey])) return [];
  const before = new Set(baseline.handles.filter(row => row.pid === process.pid).map(row => row[baselineKey]));
  const after = postExit?.handles ?? [];
  const readers = after.filter(row => row.pid === process.pid && row.pipeId && row.endpoint && row[baselineKey] && !before.has(row[baselineKey]));
