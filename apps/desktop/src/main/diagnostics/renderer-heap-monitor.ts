// Ported from PwrAgnt (diagnostics/renderer-heap-monitor.ts): attaches to a
// renderer over CDP, samples Runtime.getHeapUsage on an interval, and takes a
// heap snapshot when a sample-to-sample delta crosses the threshold.
...
const defaultHeapLogger = getDiagLogger("pwrgit:heap");
...
/** Structural slice of Electron's WebContents used by the monitor —
 *  injectable so unit tests never need a real renderer. */
export type RendererHeapTarget = {
...
export class RendererHeapMonitor {
  private readonly detachListener = (_event: unknown, reason: string) => {
    ...
    this.logger.warn("debugger detached", {
      reason,
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      this.logger.info("renderer monitoring started", {
        sessionDirectory: this.session.directoryPath,
        intervalMs: this.config.intervalMs
      });
      ...
      this.logger.error("renderer monitoring failed to start", error);
      this.pauseSampling();
    }
  }
  ...
  async stop(reason = "stopped"): Promise<void> {
    ...
    if (!targetDestroyed) {
      try {
        ...
        this.logger.warn("renderer target destroyed during stop", {
          reason,
          sessionDirectory: this.session.directoryPath
        });
      }
    ...
    this.logger.info("renderer monitoring stopped", {
      reason,
      sessionDirectory: this.session.directoryPath
    });
  }
  ...
  private async captureSample(forceBaseline: boolean): Promise<void> {
    ...
    try {
      ...
      const sample: HeapSessionSample = {
        ...
        ...(heapUsage.embedderHeapUsedSize !== undefined
          ? { embedderHeapUsedSize: heapUsage.embedderHeapUsedSize }
          : {}),
        ...(heapUsage.backingStorageSize !== undefined
      ...
      this.logger.error("renderer heap sample failed", error);
    }
  }
  ...
  private async captureSnapshot(options: {
    ...
    await this.appendEvent({
      ...
      detail: { filename: options.filename, deltaBytes: options.deltaBytes }
    });
    this.logger.warn("capturing renderer heap snapshot", {
    ...
    try {
      ...
      await this.appendEvent({
        ...
        detail: { filename: options.filename }
      });
    } catch (error) {
      await this.appendEvent({
        source: "renderer",
        ...
        detail: { filename: options.filename, error: serializeError(error) }
      });
      this.logger.error("renderer heap snapshot failed", error);
    } finally {
