// Ported from PwrAgnt (diagnostics/heap-session.ts): one directory per heap
// monitoring session holding a manifest, ndjson samples/events, and any
// captured .heapsnapshot files.
...
export async function createHeapSession(options: {
  ...
  /** Distinguishes concurrent sessions, e.g. "main" vs a window id. */
  label: string;
  createdAt?: Date;
  sessionId?: string;
  versions: HeapSessionVersions;
  ...
  const directoryName = `heap-${options.label}-${formatSessionPrefix(createdAt)}-${sessionId}`;
