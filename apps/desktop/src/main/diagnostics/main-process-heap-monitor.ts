// Ported from PwrAgnt (diagnostics/main-process-heap-monitor.ts): samples the
// main process heap on an interval and writes a V8 heap snapshot when a
// sample-to-sample delta crosses the configured threshold.
  ...
  private async captureSnapshot(options: {
    ...
    try {
      ...
      await this.appendEvent({
        ...
        detail: { filename: writtenFilename }
      });
    } catch (error) {
      await this.appendEvent({
        source: "main",
