...
// taken on faith: `PwrGit Setup.exe` and `PwrGit Setup Arm.exe` came back as
// `PwrGit.Setup.exe` and `PwrGit.Setup.Arm.exe`, and a PATCH renaming one to
// `PwrGit Setup.exe` returned `PwrGit.Setup.exe`. See
...
//
// These deliberately do not match the macOS aliases (`PwrGit.dmg`,
// `PwrGit-arm64.dmg`). Those URLs are already published and must keep working,
// and converging them would produce `PwrGit.arm64.dmg`, which is no closer to
// either platform's convention. Each platform keeps its own spelling.
export const WINDOWS_ALIAS_NAMES = {
  x64: "PwrGit.Setup.exe",
  arm64: "PwrGit.Setup.Arm.exe",
};
 ...
 * so `releases/latest/download/PwrGit.Setup.exe` keeps resolving for as long as
 * the product exists. The versioned installer stays exactly where it is:
...
// Invoked as a release-workflow step rather than from release.mjs: the alias
// must be cut after the signing job has verified Authenticode, which happens
// once packaging has already returned. Takes the release stage, the same
