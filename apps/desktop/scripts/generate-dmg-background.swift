...
func renderBackground() -> NSBitmapImageRep {
  ...
  let gitSize = "Git".size(withAttributes: [.font: logoFont])
  let logoX = (w - pwrSize.width - gitSize.width) / 2
  ...
  "Git".draw(at: NSPoint(x: logoX + pwrSize.width, y: logoY), withAttributes: accentAttrs)
